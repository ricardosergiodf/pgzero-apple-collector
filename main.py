...
def draw_menu():
    screen.draw.text("Apple Collector", center=(WIDTH//2, 150), fontsize=60, color="white", shadow=(1, 1))
    screen.draw.text("Press ENTER to Start", center=(WIDTH//2, 300), fontsize=40, color="yellow")
    screen.draw.text("Press ESC to Exit", center=(WIDTH//2, 400), fontsize=40, color="yellow")
    
    sound_status = "ON" if sound_on else "OFF"
    screen.draw.text(f"Sound: {sound_status} (Press M to toggle)", center=(WIDTH//2, 350), fontsize=30, color="white")
...
def draw_game():
    ...
    # Desenha pontuação
    screen.draw.text(f"Score: {player_score}", topleft=(20, 20), fontsize=30, color="white")
...
def draw_game_over():
    screen.draw.text("GAME OVER", center=(WIDTH//2, 200), fontsize=60, color="red", shadow=(2, 2))
    screen.draw.text(f"Final Score: {player_score}", center=(WIDTH//2, 280), fontsize=40, color="white")
    screen.draw.text("Press R to Restart", center=(WIDTH//2, 350), fontsize=30, color="yellow")
    screen.draw.text("Press M for Menu", center=(WIDTH//2, 400), fontsize=30, color="yellow")
...
def reset_game():
    ...
    enemies[0].actor.pos = (WIDTH - 100, ground.top - enemies[0].actor.height/2)  # Inimigo no chão
    enemies[1].actor.pos = (200, platforms[1].top - enemies[1].actor.height/2)    # Inimigo da plataforma 1
    enemies[2].actor.pos = (500, platforms[2].top - enemies[2].actor.height/2)    # Inimigo da plataforma 2
